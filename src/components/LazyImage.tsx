...
const LazyImage: React.FC<LazyImageProps> = ({
  ...
  // Detectar mobile e forçar carregamento imediato
  useEffect(() => {
    const checkMobile = () => {
      const mobile = window.innerWidth <= 768;
      setIsMobile(mobile);
      
      // Em mobile, sempre carrega imediatamente - SEM lazy loading
      if (mobile || priority) {
        setIsInView(true);
        console.log('Mobile detected - Loading image immediately:', src);
        return;
      }
    };
  ...
  useEffect(() => {
    // Se é mobile ou priority, não usa intersection observer
